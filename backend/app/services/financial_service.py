...
class FinancialService:
    ...
    Persistent Caching Enforced: Reuses cached analysis results across reloads and route changes.
    """
    ...
    def generate_summary(self, report_id: str, user_id: str, force_refresh: bool = False) -> FinancialSummaryResponse:
        """Generates structured executive summary and key highlights from report chunks."""
        cache_key = self.cache_service.build_cache_key("fin_summary", user_id=user_id, document_id=report_id)
        ...
        with lock:
            ...
            self.cache_service.set_pending_status(cache_key, "fin_summary", user_id, [report_id])
            ...
            try:
                ...
                self.cache_service.save_result(cache_key, "fin_summary", user_id, [report_id], res_dict)
                return res
    ...
    def extract_chart_data(self, report_id: str, user_id: str, force_refresh: bool = False) -> FinancialChartDataResponse:
        ...
        with lock:
            cached = self.cache_service.get_cached_result(cache_key, force_refresh=force_refresh)
            if cached:
                return FinancialChartDataResponse(**cached)
            ...
            try:
                ...
                if res and res.has_data:
                    res_dict = res.model_dump() if hasattr(res, "model_dump") else res.dict()
                    self.cache_service.save_result(cache_key, "fin_charts", user_id, [report_id], res_dict)
    ...
    def get_financial_overview(self, report_id: str, user_id: str, force_refresh: bool = False) -> FinancialOverviewResponse:
        """
        ...
        """
        cache_key = self.cache_service.build_cache_key("fin_overview", user_id=user_id, document_id=report_id)
        lock = self.cache_service.get_lock_for_key(cache_key)

        with lock:
            cached = self.cache_service.get_cached_result(cache_key, force_refresh=force_refresh)
            if cached:
                return FinancialOverviewResponse(**cached)

            self.cache_service.set_pending_status(cache_key, "fin_overview", user_id, [report_id])
...
{context_text}"""
                ...
                if res and res.has_data:
                    res_dict = res.model_dump() if hasattr(res, "model_dump") else res.dict()
                    self.cache_service.save_result(cache_key, "fin_overview", user_id, [report_id], res_dict)
    ...
    def analyze_financial_risks(self, report_id: str, user_id: str, force_refresh: bool = False) -> RiskAnalysisResponse:
        """
        ...
        """
        cache_key = self.cache_service.build_cache_key("fin_risks", user_id=user_id, document_id=report_id)
        lock = self.cache_service.get_lock_for_key(cache_key)
        ...
        with lock:
            ...
            self.cache_service.set_pending_status(cache_key, "fin_risks", user_id, [report_id])
...
{context_text}"""
                ...
                if res and res.total_risks_count >= 0:
                    res_dict = res.model_dump() if hasattr(res, "model_dump") else res.dict()
                    self.cache_service.save_result(cache_key, "fin_risks", user_id, [report_id], res_dict)
